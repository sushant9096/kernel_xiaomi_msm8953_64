...
turn_off:
	...
	pr_debug("XY++ %s %d led_name=%s", __func__, __LINE__, flash_node->cdev.name);
	rc = qpnp_led_masked_write(led->spmi_dev,
			FLASH_LED_STROBE_CTRL(led->base),
			(flash_node->id == FLASH_LED_SWITCH ? FLASH_STROBE_MASK
						| FLASH_LED_STROBE_TYPE_HW
						...
						enum led_brightness value)
{
	...
	pr_debug("XY++ qpnp_flashlight_led_brightness_set value=%d\n", value);
						...
						enum led_brightness value)
{
	...
	flash_node = container_of(led_cdev, struct flash_node_data, cdev);
	led = dev_get_drvdata(&flash_node->spmi_dev->dev);
	pr_debug("XY++ qpnp_flash_led_brightness_set IN\n");
...
static int qpnp_flash_led_probe(struct spmi_device *spmi)
{
	...
	for_each_child_of_node(node, temp) {
		...
		pr_debug("XY++ cdev_name[%d]=%s", i, led->flash_node[i].cdev.name);
		if (rc < 0) {
			dev_err(&led->spmi_dev->dev,
